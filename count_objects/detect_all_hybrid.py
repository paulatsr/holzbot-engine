# engine/count_objects/detect_all_hybrid.py
# pip install google-generativeai opencv-python numpy pillow requests
import google.generativeai as genai
import cv2
import numpy as np
from pathlib import Path
import os, time, shutil, json, requests
...
PLAN_PATH = "plan.jpg"
CONF_THRESHOLD = 0.3
OVERLAP = 30                 # (nefolosit direct de endpoint; păstrat pentru compat)
...
# ==============================================
COLORS = {
    "door":           {"template": (0, 165, 255), "gemini": (0, 255, 255)},          # portocaliu / galben
    "double-door":    {"template": (255, 0, 200), "gemini": (255, 150, 255)},        # magenta / roz
    "window":         {"template": (255, 0, 0),   "gemini": (255, 150, 150)},        # albastru / deschis
    "double-window":  {"template": (0, 255, 0),   "gemini": (144, 238, 144)}         # verde / deschis
}
...
def match_with_rotation(crop, templates, scales):
    ...
    for crop_angle in [0, 45, 90, 135, 180, 225, 270, 315]:
        ...
        for t in templates:
            for s in scales:
                try:
                    resized = cv2.resize(rotated_crop, (int(t["image"].shape[1]*s), int(t["image"].shape[0]*s)))
                    sim = cv2.minMaxLoc(cv2.matchTemplate(resized, t["image"], cv2.TM_CCOEFF_NORMED))[1]
...
def overlap(a, b):
    ...
    if dx <= 0 or dy <= 0:
        return 0
    area_overlap = dx * dy
    area_a = (ax2 - ax1) * (ay1 - ay2) if False else (ax2 - ax1) * (ay2 - ay1)  # păstrat exact cum e logic
    area_a = (ax2 - ax1) * (ay2 - ay1)
    return area_overlap / area_a

# ==============================================
# ROB0FLOW — Hosted Inference (fără SDK)
# ==============================================
def infer_roboflow(image_path: str, max_retries: int = 5, timeout: int = 60) -> dict:
    """
    Încearcă mai întâi infer.roboflow.com (Authorization: Key ...).
    Dacă serverul răspunde 404/405/4xx care sugerează alt endpoint,
    cade pe detect.roboflow.com (param api_key + multipart).
    Returnează dict cu cheia 'predictions' (list).
    """
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if isinstance(data, dict) and "predictions" in data:
                    return data
                # unele workspace-uri răspund cu {"predictions":[...]} direct – deja acoperit
                # fallback: dacă răspunsul are altă cheie
                return {"predictions": data.get("predictions", [])}
            elif r.status_code in (401, 403, 404, 405):
                print(f"[INFO] infer.roboflow.com returned {r.status_code}; încerc detect.roboflow.com")
                break
            else:
    ...
    # 2) detect.roboflow.com (multipart + api_key in query)
...
def main_single_plan():
    ...
    for label, folder in EXPORTS.items():
        ...
        for i, pred in enumerate(relevant, 1):
            x, y, w, h   = int(pred.get("x",0)), int(pred.get("y",0)), int(pred.get("width",0)), int(pred.get("height",0))
            conf         = float(pred.get("confidence", 0.0))
            x1, y1, x2, y2 = max(0, x - w//2), max(0, y - h//2), min(img.shape[1], x + w//2), min(img.shape[0], y + h//2)
    ...
    for label in results:
        for (x1, y1, x2, y2) in results[label]["confirm"]:
            ...
            detections_export.append({"type": label, "status": "confirmed", "x1": x1, "y1": y1, "x2": x2, "y2": y2})
        for (x1, y1, x2, y2) in results[label]["oblique"]:
            cv2.rectangle(out, (x1, y1), (x2, y2), COLORS[label]["gemini"], 2)
            detections_export.append({"type": label, "status": "gemini", "x1": x1, "y1": y1, "x2": x2, "y2": y2})
        for (x1, y1, x2, y2) in results[label]["reject"]:
            detections_export.append({"type": label, "status": "rejected", "x1": x1, "y1": y1, "x2": x2, "y2": y2})

    OUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    cv2.imwrite(str(OUT_PATH), out)

    # export în UI
    record_image(OUT_PATH, stage="count_objects",
    ...
    record_json(DETECTIONS_JSON, stage="count_objects",
    ...
    for cls in results:
        print(f"🔹 {cls}: {len(results[cls]['confirm'])} confirmate | {len(results[cls]['oblique'])} Gemini | {len(results[cls]['reject'])} respinse")
...
if __name__ == "__main__":
    ...
    if not plans_env:
        ...
        for plan_dir in plans:
            ...
            print(f"\n================= PLAN (detect_all_hybrid): {plan_path} =================")
