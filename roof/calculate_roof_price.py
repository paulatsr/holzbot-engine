...
def _house_area_path_for_plan() -> Path:
    """
    Pentru rulările multi-plan, aria fiecărui plan este salvată separat.
    Folosim acea valoare pentru a nu recalcula acoperișul pe aria cumulată.
    """
...
def _aggregate_roof_outputs() -> dict | None:
    ...
    if all("components" in data for _, data in plans_data):
        ...
        for idx, (name, data) in enumerate(plans_data, start=1):
            ...
            totals["roof_base_avg"] += float(comps["roof_base"].get("average_total_eur", 0.0))
            totals["sheet_metal"] += float(comps.get("sheet_metal", {}).get("total_eur", 0.0))
            totals["extra_walls"] += float(comps.get("extra_walls", {}).get("total_eur", 0.0))
            totals["insulation"] += float(comps.get("insulation", {}).get("total_eur", 0.0))
            totals["material"] += float(comps.get("material", {}).get("total_eur", 0.0))
            ...
            breakdown.append({
                ...
                "roof_final_total_eur": round(float(data.get("roof_final_total_eur") or 0.0), 2)
            })
    ...
    for name, data in plans_data:
        ...
        breakdown.append({
            "plan_file": name,
            "average_total_eur": round(avg_val, 2)
        })
...
# ------------------------------
# Perimetru din arie (implicit casă „aprox pătrată”): P ≈ 4 * sqrt(A)
# ------------------------------
...
# ------------------------------
# Funcția pură de calcul (NU citește fișiere)
# ------------------------------
def calc_roof_price_from_data(
    final_area_m2: float,
    roof_types: list[dict],
    roof_name_key: str,     # UI îți trimite chiar name_de (ex: "Walmdach")
    coeffs: dict,
    selected_material: str | None = None,
):
    currency = coeffs.get("currency", "EUR")

    # Perimetru: override sau calculat
    ...
    # Găsește tipul (acceptă name_de sau name_en pentru siguranță)
    roof = next((
    ...
    if not roof:
        raise ValueError(f"❌ Tipul de acoperiș '{roof_name_key}' nu a fost găsit în roof_types_germany.json")

    cost_range = roof.get("cost_estimate_eur_per_m2")
    if not isinstance(cost_range, list) or len(cost_range) != 2 or "n/a" in cost_range:
        raise ValueError(f"❌ Tipul '{roof.get('name_de', roof_name_key)}' nu are date valide pentru cost/m².")
    cmin, cmax = float(cost_range[0]), float(cost_range[1])

    # 1) Preț acoperiș (interval pe m²)
    min_roof = final_area_m2 * cmin
    max_roof = final_area_m2 * cmax
    avg_roof = (min_roof + max_roof) / 2.0

    # 2) Tinichigerie = Perimetru × Overhang(m) × Preț tinichigerie pe m
    sheet_metal_total = perimeter_m * roof_overhang_m * sheet_metal_price_per_m

    # 3) Pereți extra = Perimetru × coeficient specific tipului (€/m) — dacă lipsește, e 0
    ...
    # 4) Izolație acoperiș (pe arie) = Arie casă × coeficient izolație (€/m²)
    insulation_total = final_area_m2 * insulation_price_per_m2

    # 5) Material învelitoare (€/m²) = Arie × preț material selectat
    ...
    final_total = round(
        avg_roof + sheet_metal_total + extra_walls_total + insulation_total + material_total, 2
    )
    ...
    breakdown = {
        "meta": {
            ...
            "currency": currency,
            "perimeter_source": "override in request/coeffs" if perimeter_override is not None else "estimated as 4*sqrt(area)"
        },
        "inputs": {
...
# ------------------------------
# Integrare: citește fișiere + rulează cu un roof key din UI
# ------------------------------
def run_estimation(roof_key_from_ui: str, overrides: dict | None = None):
    ...
    sel = _read_selection()
    selected_material = (sel.get("materialAcoperis") or (overrides or {}).get("roof_material") or "").lower()

    result = calc_roof_price_from_data(final_area_m2, roof_types, roof_key_from_ui, coeffs, selected_material)
...
def main_single_plan(selected_arg: str | None = None):
    ...
    selected = selected_arg or (
        _sys.argv[1] if len(_sys.argv) > 1 else
        (sel.get("tipAcoperis") or os.getenv("ROOF_SELECTED") or "Walmdach")
    )
...
# ------------------------------
# Entry-point: CLI / ENV / MULTI_PLANS
# ------------------------------
if __name__ == "__main__":
    ...
    if not plans_env:
        ...
        for plan_dir in plans:
            ...
            print(f"\n================= PLAN (calculate_roof_price): {plan_path} =================")
