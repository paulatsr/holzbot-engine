...
def main():
    ...
    out_dir = Path(args.out_root) / "double_window"
    for i, p in enumerate([p for p in preds if "double-window" in str(p["class"]).lower()]):
        crop = crop_from_pred(img, p)
        export_rotations(crop, out_dir, f"double_window_{i}")
