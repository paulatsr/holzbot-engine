# ==============================================
# room_extraction.py (final)
# Detectează pereții, umple camerele, marchează exteriorul clar
# ==============================================
# Bootstrap ca să poți importa ui_export din folderul părinte (engine/)
import sys, pathlib
...
import json
from ui_export import record_image, record_file, record_json
...
def main_single_plan():
    ...
    walls = cv2.morphologyEx(walls, cv2.MORPH_CLOSE, np.ones(CLOSE_KERNEL, np.uint8))
    ...
    record_image(WALLS_PATH, stage="exterior_doors", caption="Pereți detectați (edge + morph).")
    record_image(ROOMS_COLORED_PATH, stage="exterior_doors", caption="Camere segmentate + exterior (gri).")
    record_file(LABELS_PATH, stage="exterior_doors", caption="Mască label-uri camere (npy).")
    record_json(ROOMS_DATA_PATH, stage="exterior_doors", caption="Meta-date camere + parametri detecție.")
...
if __name__ == "__main__":
    ...
    if not plans_env:
        ...
        for plan_dir in plans:
            ...
            print(f"\n================= PLAN (room_extraction): {plan_path} =================")
