...
def _plan_id() -> str | None:
    plan_id = (Path.cwd().name or "").strip()  # fallback nice-to-have
    # dar respectăm și env-ul dacă e setat explicit
    env = (Path.cwd().anchor and None) or None  # dummy, dar păstrăm patternul tău
    # mai simplu: doar env
    import os
    env = os.getenv("PLAN_ID")
    if env:
        env = env.strip()
        return env or None
    return None
...
def _plan_output_path(base_path: Path) -> Path | None:
    """
    Dacă ai PLAN_ID=p01, vei avea house_area_gemini_p01.json etc.
    """
    plan_id = _plan_id()
    if not plan_id:
        return None
...
def main_single_plan() -> None:
    ...
    # 📂 Fișiere
    plan_path = PLAN_IMAGE
    scale_file = SCALE_RESULT_JSON
    output_file = HOUSE_AREA_JSON

    if not scale_file.exists():
        raise FileNotFoundError(f"❌ Nu găsesc fișierul de scală: {scale_file}")
    if not plan_path.exists():
        raise FileNotFoundError(f"❌ Nu găsesc imaginea planului: {plan_path}")
    ...
    # 🧠 Prompt îmbunătățit (preluat 1:1 din codul tău)
    prompt = f"""
...
"""
    ...
    try:
        ...
        print("⚠️ Modelul gemini-2.5-pro nu e disponibil, folosesc gemini-2.5-flash.")
