...
from runner.ui_export import record_image, record_text

# Lucrăm relativ la CWD (plan folder)
ROOT = Path(".").resolve()
...
def main():
    ...
    if total == 0:
        ...
        record_text(
            f"Selectez {len(picks)} din {total} imagini (sampling uniform)",
...
if __name__ == "__main__":
    ...
    if not plans_env:
        ...
        for plan_dir in plans:
            ...
            print(f"\n================= PLAN (export_templates_from_detections): {plan_path} =================")
