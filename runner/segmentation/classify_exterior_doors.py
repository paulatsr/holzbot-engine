...
def run_room_extraction():
    """Rulează room_extraction.py dacă lipsesc fișierele. (compatibilitate)"""
...
def main_single_plan():
    ...
    if not exterior_labels:
        ...
        if not exterior_labels:
            raise ValueError(
                "❌ Nu există camere marcate ca exterior (is_exterior=true) în JSON!"
            )
        else:
    ...
    print(
        f"📏 Harta distanței față de exterior calculată corect (cu {len(exterior_labels)} labeluri)."
    )
